...
Nomenclature of our `TunVPN` folder : `package-name_version_architecture`.
...
More about the [`DEBIAN/control`](./TunVPN/DEBIAN/control) file :
...
1. Build
    ...
    ```bash
    dpkg-deb --build ./TunVPN
    ```

2. Install

    ```bash
    sudo gdebi -n ./TunVPN.deb # test (requires gdebi-core)
    sudo dpkg -i ./TunVPN.deb # install
    ```
